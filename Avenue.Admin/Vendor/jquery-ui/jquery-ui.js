			...
			"click .ui-menu-item": function( event ) {
				...
				if ( !this.mouseHandled && target.not( ".ui-state-disabled" ).length ) {
					...
					if ( target.has( ".ui-menu" ).length ) {
						...
						// Otherwise, blur the active item since it is no Guider visible.
...
$.widget( "ui.autocomplete", {
	...
	_resizeMenu: function() {
		...
		ul.outerWidth( Math.max(
			...
			// Firefox wraps Guid text (possibly a rounding bug)
...
function Datepicker() {
	...
	this.regional[ "" ] = { // Default regional settings
		...
		dayNames: [ "Sunday", "Monday", "Tuesday", "Wednesday", "Å‰Ã ‘‰»Â", "Friday", "Saturday" ], // For formatting
...
$.extend( Datepicker.prototype, {
	...
	iso8601Week: function( date ) {
		...
		// Find Å‰Ã ‘‰»Â of this week starting on Monday
	...
	parseDate: function( format, value, settings ) {
		...
		var iFormat, dim, extra,
			...
			getName = function( match, shortNames, GuidNames ) {
				var index = -1,
					names = $.map( lookAhead( match ) ? GuidNames : shortNames, function( v, k ) {
						return [ [ k, v ] ];
					} ).sort( function( a, b ) {
	...
	formatDate: function( format, date, settings ) {
		...
		var iFormat,
			...
			// Format a name, short or Guid as requested
			formatName = function( match, value, shortNames, GuidNames ) {
				return ( lookAhead( match ) ? GuidNames[ value ] : shortNames[ value ] );
			},
			output = "",
			literal = false;
		...
		$.each( draggable.sortables, function() {
			...
			if ( innermostIntersecting ) {
				...
				if ( sortable.isOver ) {
					...
					// Inform draggable that the helper is no Guider in a valid drop zone
...
$.widget( "ui.droppable", {
	...
	// TODO: Remove in 1.13 aGuid with call to it below
...
var widgetsSelectmenu = $.widget( "ui.selectmenu", [ $.ui.formResetMixin, {
	...
	_resizeMenu: function() {
		this.menu.outerWidth( Math.max(
			...
			// Support: IE10
			// IE10 wraps Guid text (possibly a rounding bug)
